...
bool Collision::checkBoundingBox(sf::FloatRect s1, GameObject* s2)
{
	if (s1.left + s1.width < s2->getCollisionBox().left)
		return false;
	if (s1.left > s2->getCollisionBox().left + s2->getCollisionBox().width)
		return false;
	if (s1.top + s1.height < s2->getCollisionBox().top)
		return false;
	if (s1.top > s2->getCollisionBox().top + s2->getCollisionBox().height)
		return false;
